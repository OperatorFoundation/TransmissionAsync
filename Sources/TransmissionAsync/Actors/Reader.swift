...
public actor Reader<T: Readable>
{
    ...
    public func read(_ size: Int) async throws -> Data
    {
        if self.verbose
        {
            self.logger.debug("Reader.read(\(size))")
        }
        ...
        if self.verbose
        {
            self.logger.debug("Reader.read(\(size)) - calling self.reader.read(\(size))")
        }
    ...
    public func readWithLengthPrefixNonblocking(_ prefixSizeInBits: Int) async throws -> Data
    {
        if verbose
        {
            self.logger.trace("Reader[\(self.reader)].readWithLengthPrefixNonblocking(\(prefixSizeInBits))")
        }
        ...
        if verbose
        {
            self.logger.trace("\(self.reader).readNonblocking(\(sizeInBytes))")
        }

        let lengthBytes = try await self.reader.readNonblocking(sizeInBytes)

        if self.verbose
        {
            self.logger.debug("Reader.readWithLengthPrefix (nonblocking) - \(lengthBytes.count) \(lengthBytes.hex) \(prefixSizeInBits)")
        }
